...
import type { Climb, Tick } from "./types.js";
...
export interface RecommendationSet {
  area: string;
  workingGrade: string;
  groups: { wall: string; climbs: Recommendation[] }[];
}
 ...
 */
export async function recommendNext(ticks: Tick[], areaUuid: string): Promise<RecommendationSet> {
  ...
  const groups = [...byWall.entries()]
    ...
    .sort((a, b) => a.bestScore - b.bestScore || a.wall.localeCompare(b.wall))
    .map(({ wall, climbs }) => ({ wall, climbs }));
